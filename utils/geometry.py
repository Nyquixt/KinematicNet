...
def mean_angle_error_pavllo(fake_rotations_full, quaternion_angles, n_joints=14):
    '''
        Input: np.arrays - (*, 4)
        Output: error - scalar
    '''

    # convert to torch for qeuler
    # predicted_quat = torch.from_numpy(fake_rotations_full).float()
    # expected_quat = torch.from_numpy(quaternion_angles).float()
    ...
    predicted_euler = qeuler(predicted_quat, order='zxy', epsilon=1e-6).numpy()

    # convert to euler zxy
    expected_quat = quaternion_angles.view(-1, n_joints, 4)
    expected_euler = qeuler(expected_quat, order='zxy', epsilon=1e-6).numpy()

    # L1 loss on angle distance with 2pi wrap-around
    angle_distance = np.remainder(predicted_euler - expected_euler + np.pi, 2 * np.pi) - np.pi
    return np.mean(np.absolute(angle_distance))

def mean_angle_error_per_joint_pavllo(fake_rotations_full, quaternion_angles, n_joints=14):
    '''
        Input: np.arrays - (*, 4)
        Output: error - scalar
    '''
    ...
    for joint in range(n_joints):
        # convert to euler zxy
        predicted_euler = qeuler(predicted_quat[:, joint], order='zxy', epsilon=1e-6).numpy()

        # convert to euler zxy
        expected_quat = quaternion_angles.view(-1, n_joints, 4)
        expected_euler = qeuler(expected_quat[:, joint], order='zxy', epsilon=1e-6).numpy()

        # L1 loss on angle distance with 2pi wrap-around
        angle_distance = np.remainder(predicted_euler - expected_euler + np.pi, 2 * np.pi) - np.pi
        error = np.mean(np.absolute(angle_distance))
...
def compute_rotation_matrix_from_ortho6d(poses):
    ...
    x = F.normalize(x_raw, 1) #batch*3
    z = torch.cross(x, y_raw, 1) #batch*3
    z = F.normalize(z, 1)#batch*3
    y = torch.cross(z, x, 1)#batch*3
...
def quat2mat(quat):
    ...
    """
    norm_quat = F.normalize(quat, dim=1)
