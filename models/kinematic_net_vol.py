...
from utils.geometry import quat2mat, compute_rotation_matrix_from_ortho6d, compute_rotation_matrix_from_euler
...
class KinematicNet(nn.Module):
    def __init__(self, config,
        ...
        self.encoder = Encoder(config.model.volume_size)

        self.regressor = Regressor(128 * 2 * 2 * 2, self.num_joints * self.rot_dim)
    ...
    def forward(self, images, proj_matricies, batch):
        ...
        for batch_i in range(batch_size):
            if self.root_mode == 'global':
                ...
                base_point = keypoints_3d[6, :3] # global root
            else:
        ...
        representation = self.regressor(volumes)

        representation = representation.view(-1, self.rot_dim) # B*J, rot_dim
        
        if self.rotation_type == 'quaternion':
            rot_mat = quat2mat(representation) # B*J, 3, 3
        elif self.rotation_type == 'euler':
            rot_mat = compute_rotation_matrix_from_euler(representation)
        elif self.rotation_type == '6d':
            rot_mat = compute_rotation_matrix_from_ortho6d(representation)
        representation = representation.view(batch_size, -1, self.rot_dim)
        rot_mat = rot_mat.view(batch_size, self.num_joints, 3, 3) # B, J, 3, 3
        return rot_mat