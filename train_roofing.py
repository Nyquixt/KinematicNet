...
from models.kinematic_net_vol import KinematicNet

from utils import cfg
from dataset import roofing
from dataset import roofing_utils as dataset_utils
from utils.geometry import rotation_matrix_to_quaternion, mean_angle_error_pavllo
...
def one_epoch(model, criterion, opt, dataloader, device, epoch, is_train=True, experiment_dir=None, verbose=False):
    if is_train:
        model.train()
    else:
        model.eval()
        ...
        if verbose:
            with tqdm(dataloader, unit='batch') as tepoch:
                for batch in tepoch:
                    ...
                    images_batch, proj_matricies_batch, rotations_batch = dataset_utils.prepare_batch(batch, device) # process batch
...
def main(args):
    ...
    model = KinematicNet(config).to(device) # define model
    ...
    if not args.eval:
        ...
        for epoch in range(config.opt.n_epochs):
            ...
            train_avg_loss = one_epoch(model, criterion, opt, train_dataloader, device, epoch, is_train=True, experiment_dir=experiment_dir, verbose=args.verbose)
            val_avg_loss, err = one_epoch(model, criterion, opt, val_dataloader, device, epoch, is_train=False, experiment_dir=experiment_dir, verbose=args.verbose)
        ...
        if args.eval_dataset == 'train':
            one_epoch(model, criterion, opt, config, train_dataloader, device, 0, is_train=False, experiment_dir=experiment_dir, verbose=args.verbose)
        else:
            one_epoch(model, criterion, opt, config, val_dataloader, device, 0, is_train=False, experiment_dir=experiment_dir, verbose=args.verbose)
